...
- **机制**:
  ...
  4. **动态颜色扩展 (v0.1.11+)**: 如果颜色循环用尽，它不再简单地重复第一种颜色，而是通过 HLS (Hue-Lightness-Saturation) 空间变换自动生成新颜色。它会基于原始颜色循环中的基础色，通过交替增加和减少亮度（Lightness）来生成同色系的变体，确保在绘制包含 10 个以上系列的复杂图表时，每个系列依然拥有唯一的视觉标识。
